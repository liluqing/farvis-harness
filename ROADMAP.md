...
**Step 1 已完成（v0.5.0）**：Skill 改造 + 异常场景处理机制 + 文档管理体系 + 整合 + 渐进式发现。
...
- **第四轮**：渐进式发现——将 `flow/skill.md` 从大而全手册改为短入口，拆出 `discovery.md`、`protocols/`、`routes/`、`lifecycle/`，让 Agent 按场景加载细节。
...
| 1.12 | 渐进式发现拆分 | 将大 Skill 拆为短入口 + discovery/protocols/routes/lifecycle/phase 文件 | Agent 只加载当前场景需要的说明，入口更短、路由更清楚、上下文占用更低 | ✅ |
...
当前版本：**v0.5.0**（Step 1 完成：流程对齐 + 异常场景处理 + 文档管理体系 + 整合 + 渐进式发现）
