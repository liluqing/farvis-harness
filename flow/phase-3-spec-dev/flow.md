...
3. 输出切片概要后等用户回复「确认」再开始，不自动跳过

**确认规则：**
- Phase 2 已经确认过任务清单 → Phase 3 的切片拆解是**细化**，不是重新确认
- ≤ 3 切片 → 输出切片概要，**必须等用户回复「确认」后再开始**（规则同 >3 切片）
- > 3 切片 → 输出完整切片列表，用户 review 后开始
- **无论切片数多少，第一版切片列表必须让用户看过。** 这是人机协作的安全阀
...
下一步：
- Phase 4（集成测试）需要你先确认再进入
- 测试计划概要等下给你看
```

**Phase 3 → Phase 4 过渡规则：Agent 必须等用户回复"进入 Phase 4"或"继续"后才加载 Phase 4 flow.md。禁止自动跳转。**

### 退出前多仓库检查

Phase 3 退出前，Agent 必须对所有关联仓库执行 git status：

```bash
# 如果 core-design/repos.yaml 存在，读取仓库列表
# 否则至少检查当前仓库
...
- 任何未提交代码 → `git add -A && git commit -m "fix: Phase 3 完成，对齐文档"`
- 提交后推送：`git push origin <当前分支>`
- 多仓库按 `core-design/repos.yaml` 顺序逐一执行

### 文档-代码对齐自检

Phase 3 所有切片完成后，在退出前执行：

1. 读所有迭代文档（`api-changes.md`、`ddl-changes.md`、`tech-design.md`）
2. 对比实际代码 — Controller 返回体、Entity 方法名、接口参数、分页格式
3. 发现不一致 → 更新文档（代码为准）
4. 更新后 `git commit -m "fix: Phase 3 文档-代码对齐"`
