...
- [ ] 环境自检：`docker compose ps`（无 running 容器则自动 `docker compose up -d`）

> 如果 `docker compose` 不可用，手动检查进程和端口。
...
| ai-context 文件 | 检查内容 |
|----------------|----------|
| `.harness/ai-context/project-map.yaml` 或 `.harness/ai-context/context.yaml` | 新增/删除的模块、API 端点变化、依赖关系变化 |
...
5. 如果使用单文件版 `.harness/ai-context/context.yaml`，同步更新对应 section
